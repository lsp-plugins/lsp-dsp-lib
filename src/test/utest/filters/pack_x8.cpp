...
namespace lsp
{
    ...
    typedef void (*biquad_pack_x8_t)(dsp::biquad_x8_t *dst, const dsp::biquad_x1_t *src);
}

UTEST_BEGIN("dsp.filters", pack_x8)

    void call(const char *label, biquad_pack_x8_t func)
    {
        if (!UTEST_SUPPORTED(func))
            return;

        FloatBuffer src((sizeof(dsp::biquad_x1_t) / sizeof(float)) * FILTER_TIMES);
        FloatBuffer dst1(sizeof(dsp::biquad_x8_t) / sizeof(float), LSP_DSP_BIQUAD_ALIGN);
        FloatBuffer dst2(sizeof(dsp::biquad_x8_t) / sizeof(float), LSP_DSP_BIQUAD_ALIGN);
        ...
        generic::biquad_pack_x8(dst1.data<dsp::biquad_x8_t>(), f);
        func(dst2.data<dsp::biquad_x8_t>(), f);
