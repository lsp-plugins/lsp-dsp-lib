...
UTEST_BEGIN("dsp.filters", pack_x4)

    void call(const char *label, biquad_pack_x4_t func)
    {
        if (!UTEST_SUPPORTED(func))
            return;

        FloatBuffer src((sizeof(dsp::biquad_x1_t) / sizeof(float)) * FILTER_TIMES);
        FloatBuffer dst1(sizeof(dsp::biquad_x4_t) / sizeof(float), LSP_DSP_BIQUAD_ALIGN);
        FloatBuffer dst2(sizeof(dsp::biquad_x4_t) / sizeof(float), LSP_DSP_BIQUAD_ALIGN);
        ...
        generic::biquad_pack_x4(dst1.data<dsp::biquad_x4_t>(), f);
        func(dst2.data<dsp::biquad_x4_t>(), f);
