...
UTEST_BEGIN("dsp.complex", mod)
    ...
    void call(const char *text,  size_t align, complex_mod_t func)
    {
                ...
                32, 33, 37, 48, 49, 64, 65, 0x3f, 100, 999, 0x1fff)
        {
            for (size_t mask=0; mask <= 0x07; ++mask)
            {
                ...
                // Call functions
                generic::complex_mod(dst1, src_re, src_im, count);
    ...
    UTEST_MAIN
    {
        ...
        IF_ARCH_X86(CALL(avx::complex_mod, 32));
        IF_ARCH_X86(CALL(avx::complex_mod_fma3, 32));
        IF_ARCH_X86(CALL(avx512::complex_mod, 64));
