...
namespace lsp
{
    ...
    typedef void (* clamp_vv2_t)(float *dst, const float *src, const float *min, const float *max, size_t count);
}

//-----------------------------------------------------------------------------
// Performance test
PTEST_BEGIN("dsp.pmath", clamp_vv2, 5, 1000)

    void call(const char *label, float *dst, const float *src, const float *min, const float *max, size_t count, clamp_vv2_t func)
    {
    ...
    PTEST_MAIN
    {
        ...
        float *min      = &src[buf_size];
        float *max      = &min[buf_size];

        randomize(dst, buf_size, 0.0f, 1.0f);
        randomize(min, buf_size, 0.2f, 0.25f);
        randomize(max, buf_size, 0.75f, 0.8f);

        #define CALL(func) \
            dsp::copy(dst, src, buf_size); \
            call(#func, dst, src, min, max, count, func);
