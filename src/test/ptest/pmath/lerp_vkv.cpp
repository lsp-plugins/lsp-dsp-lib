...
//-----------------------------------------------------------------------------
// Performance test
PTEST_BEGIN("dsp.pmath", lerp_vkv, 5, 1000)

    void call(const char *label, float *dst, const float *a, float b, const float *k, size_t count, lerp_vkv_t func)
    {
    ...
    PTEST_MAIN
    {
        ...
        #define CALL(func) \
            call(#func, dst, a, 1.0f, k, count, func);
        ...
        for (size_t i=MIN_RANK; i <= MAX_RANK; ++i)
        {
            ...
            CALL(generic::lerp_vkv);
            IF_ARCH_X86(CALL(sse::lerp_vkv));
            IF_ARCH_X86(CALL(avx::lerp_vkv));
            IF_ARCH_X86(CALL(avx::lerp_vkv_fma3));
            IF_ARCH_X86(CALL(avx512::lerp_vkv));
            IF_ARCH_ARM(CALL(neon_d32::lerp_vkv));
            IF_ARCH_AARCH64(CALL(asimd::lerp_vkv));
            PTEST_SEPARATOR;
        }
