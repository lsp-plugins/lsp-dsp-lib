...
//-----------------------------------------------------------------------------
// Performance test
PTEST_BEGIN("dsp.pmath", pmix_v1, 5, 1000)

    void call(const char *label, float *dst, const float *src, const float *k, size_t count, pmix_v1_t func)
    {
    ...
    PTEST_MAIN
    {
        ...
        float *k        = &src[buf_size];
        float *backup   = &k[buf_size];

        randomize(dst, buf_size, -1.0f, 1.0f);
        randomize(src, buf_size, -1.0f, 1.0f);
        randomize(k, buf_size, 0.25f, 0.75f);
        dsp::copy(backup, dst, buf_size);

        #define CALL(func) \
            dsp::copy(dst, backup, buf_size); \
            call(#func, dst, src, k, count, func);
