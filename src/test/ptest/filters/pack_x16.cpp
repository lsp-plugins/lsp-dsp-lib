...
PTEST_BEGIN("dsp.filters", pack_x16, 5, 1000)

    void call(const char * label, dsp::biquad_x16_t *dst, const dsp::biquad_x1_t *src, biquad_pack_x16_t func)
    {
        printf("Testing %s...\n", label);

        PTEST_LOOP(label,
            dsp::biquad_x16_t *d = dst;
    ...
    PTEST_MAIN
    {
        ...
        dsp::biquad_x16_t * const dst = alloc_aligned<dsp::biquad_x16_t>(p1, NUM_FILTERS, LSP_DSP_BIQUAD_ALIGN);
