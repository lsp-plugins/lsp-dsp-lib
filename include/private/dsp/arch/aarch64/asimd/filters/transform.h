...
#ifndef PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_TRANSFORM_H_
#define PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_TRANSFORM_H_
    ...
    namespace asimd
    {
        ...
        void bilinear_transform_x16(dsp::biquad_x16_t *bf, const dsp::f_cascade_t *bc, float kf, size_t count)
        {
            ...
            (
                ...
                BILINEAR_TRANSFORM_X8
                __ASM_EMIT("stp             q0, q24, [%[bf], #0x00]")
                __ASM_EMIT("stp             q2, q26, [%[bf], #0x40]")
                __ASM_EMIT("stp             q4, q28, [%[bf], #0x80]")
                __ASM_EMIT("stp             q6, q30, [%[bf], #0xc0]")
                __ASM_EMIT("stp             q7, q31, [%[bf], #0x100]")
                __ASM_EMIT("add             %[bc], %[bc], #0x100")

                // Step 2
                BILINEAR_TRANSFORM_X8
                __ASM_EMIT("stp             q0, q24, [%[bf], #0x20]")
                __ASM_EMIT("stp             q2, q26, [%[bf], #0x60]")
                __ASM_EMIT("stp             q4, q28, [%[bf], #0xa0]")
                __ASM_EMIT("stp             q6, q30, [%[bf], #0xe0]")
                __ASM_EMIT("stp             q7, q31, [%[bf], #0x120]")
                __ASM_EMIT("add             %[bc], %[bc], #0x100")

                __ASM_EMIT("subs            %[count], %[count], #1")
                __ASM_EMIT("add             %[bf], %[bf], #0x140")
                __ASM_EMIT("b.hs            1b")
                __ASM_EMIT("2:")
...
#endif /* PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_TRANSFORM_H_ */
