...
#ifndef PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_STATIC_H_
#define PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_STATIC_H_
...
namespace lsp
{
    namespace asimd
    {
        void biquad_process_x1(float *dst, const float *src, float *d, size_t count, const dsp::biquad_x1_t *f)
        {
            ...
            // d1'   = b2*s + a2*s';
            ARCH_AARCH64_ASM(
                __ASM_EMIT("ld3             {v16.s, v17.s, v18.s}[0], [%[f]]")      // v16  = b0, v17 = b1, v18 = b2
                __ASM_EMIT("ldp             s19, s20, [%[f], #0x0c]")               // v19  = a1, v20 = a2
                ...
                __ASM_EMIT("fmul            s0, s18, s0")                           // v0   = b2*s0
                __ASM_EMIT("fadd            s22, s4, s6")                           // v22  = d0' = d1 + b1*s0 + a1*s'
                __ASM_EMIT("fmadd           s23, s20, s2, s0")                      // v23  = d1' = b2*s0 + a2*s'
                __ASM_EMIT("fmadd           s3, s16, s1, s22")                      // v3   = s' = b0*s1+d0
                __ASM_EMIT("fmul            s5, s17, s1")                           // v5   = b1*s1
                __ASM_EMIT("fmadd           s7, s19, s3, s23")                      // v7   = a1*s3 + d1
                __ASM_EMIT("fmul            s1, s18, s1")                           // v1   = b2*s1
        ...
        void biquad_process_x2(float *dst, const float *src, float *d, size_t count, const dsp::biquad_x2_t *f)
        {
            ...
            ARCH_AARCH64_ASM(
                ...
                __ASM_EMIT("ldp             d16, d18, [%[f], #0x00]")               // v16  = b0, v18 = b1
                __ASM_EMIT("ldp             d19, d20, [%[f], #0x10]")               // v19  = b2, v20 = a1
                __ASM_EMIT("ldr             d21, [%[f], #0x20]")                    // v21  = a2
                // x1 head block
                __ASM_EMIT("ld1             {v0.s}[0], [%[src]], #4")               // v0   = s0
                ...
                __ASM_EMIT("fmul            v6.2s, v19.2s, v0.2s")                  // v6   = b2*s0
                __ASM_EMIT("fmla            v5.2s, v20.2s, v4.2s")                  // v5   = d0' = d1+b1*s0+a1*s'
                __ASM_EMIT("fmla            v6.2s, v21.2s, v4.2s")                  // v6   = d1' = b2*s0+a2*s'
                ...
                __ASM_EMIT("b.ls            2f")
                __ASM_EMIT("1:")
                __ASM_EMIT("ld1             {v0.s}[0], [%[src]], #4")               // v0   = s0 j0
                ...
                __ASM_EMIT("st1             {v4.s}[1], [%[dst]], #4")
                __ASM_EMIT("fadd            v22.2s, v23.2s, v2.2s")                 // v22  = d1+b1*s0 e1+b1*j0
                __ASM_EMIT("fmul            v23.2s, v19.2s, v0.2s")                 // v23  = b2*s0 b2*j0
                __ASM_EMIT("subs            %[count], %[count], #1")
                __ASM_EMIT("fmla            v22.2s, v20.2s, v4.2s")                 // v22  = d0' e0' = d1+b1*s0+a1*s' e1+b1*j0+a1*j'
                __ASM_EMIT("fmla            v23.2s, v21.2s, v4.2s")                 // v23  = d1' e1' = b2*s0+a2*s' b2*j0 a2*j'
                __ASM_EMIT("mov             v0.s[1], v4.s[0]")                      // shift
                __ASM_EMIT("b.hi            1b")
                __ASM_EMIT("2:")
                // x1 tail block:
                ...
                __ASM_EMIT("st1             {v4.s}[1], [%[dst]], #4")
                __ASM_EMIT("fmul            v6.2s, v19.2s, v0.2s")                  // v6   = b2*j0
                __ASM_EMIT("fmla            v5.2s, v20.2s, v4.2s")                  // v5   = d0' = e1 + b1*j0 + a1*j'
                __ASM_EMIT("fmla            v6.2s, v21.2s, v4.2s")                  // v6   = d1' = b2*j0 + a2*j'
        ...
        void biquad_process_x4(float *dst, const float *src, float *d, size_t count, const dsp::biquad_x4_t *f)
        {
            ...
            (
                ...
                __ASM_EMIT("ldp         q18, q19, [%[f], #0x00]")               // q18  = b0, q19 = b1
                __ASM_EMIT("ldp         q20, q21, [%[f], #0x20]")               // q20  = b2, q21 = a1
                __ASM_EMIT("ldr         q22, [%[f], #0x40]")                    // q22  = a2
                __ASM_EMIT("ldp         q24, q25, [%[X_MASK]]")                 // q24  = vmask, q25 = 1
                __ASM_EMIT("mov         %[mask], #1")                           // mask = 0

                // Do pre-loop
                __ASM_EMIT("1:")
                __ASM_EMIT("ld1         {v0.s}[0], [%[src]], #4")               // v0   = s
                __ASM_EMIT("fmul        v1.4s, v18.4s, v0.4s")                  // v1   = b0*s
                __ASM_EMIT("fmul        v2.4s, v19.4s, v0.4s")                  // v2   = b1*s
                __ASM_EMIT("fmul        v3.4s, v20.4s, v0.4s")                  // v3   = b2*s
                __ASM_EMIT("fadd        v0.4s, v1.4s, v16.4s")                  // v0   = b0*s + d0 = s'
                __ASM_EMIT("fmla        v2.4s, v21.4s, v0.4s")                  // v2   = b1*s + a1*s'
                __ASM_EMIT("fmla        v3.4s, v22.4s, v0.4s")                  // v3   = d1' = b2*s + a2*s'
                __ASM_EMIT("fadd        v2.4s, v2.4s, v17.4s")                  // v2   = d0' = d1 + b1*s + a1*s'
                __ASM_EMIT("subs        %[count], %[count], #1")
                ...
                __ASM_EMIT("subs        %[count], %[count], #1")
                __ASM_EMIT("ext         v0.16b, v0.16b, v0.16b, #12")           // v0   = s' = s[3] s[0] s[1] s[2]
                __ASM_EMIT("fadd        v16.4s, v2.4s, v17.4s")                 // v16  = d0' = d1 + b1*s + a1*s'
                __ASM_EMIT("mov         v17.16b, v3.16b")                       // v17  = d1'
                __ASM_EMIT("st1         {v0.s}[0], [%[dst]], #4")
                __ASM_EMIT("b.ne        5b")
        ...
        void biquad_process_x8(float *dst, const float *src, float *d, size_t count, const dsp::biquad_x8_t *f)
        {
            ...
            (
                ...
                __ASM_EMIT("ldp             q16, q17, [%[f], #0x00]")                   // q16-q17  = b0
                __ASM_EMIT("ldp             q18, q19, [%[f], #0x20]")                   // q18-q19  = b1
                __ASM_EMIT("ldp             q20, q21, [%[f], #0x40]")                   // q20-q21  = b2
                __ASM_EMIT("ldp             q22, q23, [%[f], #0x60]")                   // q22-q23  = a1
                __ASM_EMIT("ldp             q24, q25, [%[f], #0x80]")                   // q24-q25  = a2
                // Do pre-loop
                __ASM_EMIT("1:")
                __ASM_EMIT("ld1             {v0.s}[0], [%[src]], #4")                   // q0       = s
                __ASM_EMIT("fmul            v2.4s, v16.4s, v0.4s")                      // v2       = b0*s
                __ASM_EMIT("fmul            v3.4s, v17.4s, v1.4s")
                __ASM_EMIT("fmul            v4.4s, v18.4s, v0.4s")                      // v4       = b1*s
                __ASM_EMIT("fmul            v5.4s, v19.4s, v1.4s")
                __ASM_EMIT("fmul            v6.4s, v20.4s, v0.4s")                      // v6       = b2*s
                __ASM_EMIT("fmul            v7.4s, v21.4s, v1.4s")
                __ASM_EMIT("fadd            v0.4s, v2.4s, v8.4s")                       // v0       = b0*s + d0 = s'
                __ASM_EMIT("fadd            v1.4s, v3.4s, v9.4s")
                __ASM_EMIT("fmla            v4.4s, v22.4s, v0.4s")                      // v4       = b1*s + a1*s'
                __ASM_EMIT("fmla            v5.4s, v23.4s, v1.4s")
                __ASM_EMIT("fmla            v6.4s, v24.4s, v0.4s")                      // v6       = d1' = b2*s + a2*s'
                __ASM_EMIT("fmla            v7.4s, v25.4s, v1.4s")
                __ASM_EMIT("fadd            v4.4s, v4.4s, v10.4s")                      // v4       = d0' = d1 + b1*s + a1*s'
                ...
                __ASM_EMIT("fmla            v6.4s, v24.4s, v0.4s")                      // v6       = d1' = b2*s + a2*s'
                __ASM_EMIT("fmla            v7.4s, v25.4s, v1.4s")
                __ASM_EMIT("fadd            v8.4s, v4.4s, v10.4s")                      // v8       = d0' = d1 + b1*s + a1*s'
                __ASM_EMIT("fadd            v9.4s, v5.4s, v11.4s")
                __ASM_EMIT("subs            %[count], %[count], #1")
                __ASM_EMIT("st1             {v1.s}[3], [%[dst]], #4")                   // *dst     = s'[7]
        ...
        void biquad_process_x16(float *dst, const float *src, float *d, size_t count, const dsp::biquad_x16_t *f)
        {
            ...
            (
                ...
                // Cycle 1
                // Prepare
                __ASM_EMIT("str             %[dst], [%[backup], #0x00]")
                __ASM_EMIT("str             %[count], [%[backup], #0x08]")
                __ASM_EMIT("ldp             q8, q9, [%[d], #0x00]")                     // v8-v9    = d0
                __ASM_EMIT("ldp             q10, q11, [%[d], #0x40]")                   // v10-v11  = d1
                __ASM_EMIT("mov             %[mask], #1")                               // mask     = 1
                __ASM_EMIT("ldp             q26, q27, [%[X_MASK], #0x00]")              // v26-v27  = vmask
                __ASM_EMIT("ldp             q28, q29, [%[X_MASK], #0x20]")              // v28-v29  = vshift = 1
                __ASM_EMIT("ldp             q16, q17, [%[f], #0x00]")                   // q16-q17  = b0
                __ASM_EMIT("ldp             q18, q19, [%[f], #0x40]")                   // q18-q19  = b1
                __ASM_EMIT("ldp             q20, q21, [%[f], #0x80]")                   // q20-q21  = b2
                __ASM_EMIT("ldp             q22, q23, [%[f], #0xc0]")                   // q22-q23  = a1
                __ASM_EMIT("ldp             q24, q25, [%[f], #0x100]")                  // q24-q25  = a2
                // Do pre-loop
                __ASM_EMIT("1:")
                __ASM_EMIT("ld1             {v0.s}[0], [%[src]], #4")                   // q0       = s
                __ASM_EMIT("fmul            v2.4s, v16.4s, v0.4s")                      // v2       = b0*s
                ...
                __ASM_EMIT("6:")
                __ASM_EMIT("stp             q8, q9, [%[d], #0x00]")                     // v8-v9    = d0
                __ASM_EMIT("stp             q10, q11, [%[d], #0x40]")                   // v10-v11  = d1

                //-------------------------------------------------------------
                // Cycle 2
                // Prepare
                __ASM_EMIT("ldr             %[dst], [%[backup], #0x00]")
                __ASM_EMIT("ldr             %[count], [%[backup], #0x08]")
                __ASM_EMIT("mov             %[src], %[dst]")
                __ASM_EMIT("ldp             q8, q9, [%[d], #0x20]")                     // v8-v9    = d0
                __ASM_EMIT("ldp             q10, q11, [%[d], #0x60]")                   // v10-v11  = d1
                __ASM_EMIT("mov             %[mask], #1")                               // mask     = 1
                __ASM_EMIT("ldp             q26, q27, [%[X_MASK], #0x00]")              // v26-v27  = vmask
                __ASM_EMIT("ldp             q28, q29, [%[X_MASK], #0x20]")              // v28-v29  = vshift = 1
                __ASM_EMIT("ldp             q16, q17, [%[f], #0x20]")                   // q16-q17  = b0
                __ASM_EMIT("ldp             q18, q19, [%[f], #0x60]")                   // q18-q19  = b1
                __ASM_EMIT("ldp             q20, q21, [%[f], #0xa0]")                   // q20-q21  = b2
                __ASM_EMIT("ldp             q22, q23, [%[f], #0xe0]")                   // q22-q23  = a1
                __ASM_EMIT("ldp             q24, q25, [%[f], #0x120]")                  // q24-q25  = a2
                // Do pre-loop
                __ASM_EMIT("1:")
                __ASM_EMIT("ld1             {v0.s}[0], [%[src]], #4")                   // q0       = s
                __ASM_EMIT("fmul            v2.4s, v16.4s, v0.4s")                      // v2       = b0*s
                ...
                // Store memory
                __ASM_EMIT("6:")
                __ASM_EMIT("stp             q8, q9, [%[d], #0x20]")                     // v8-v9    = d0
                __ASM_EMIT("stp             q10, q11, [%[d], #0x60]")                   // v10-v11  = d1
...
#endif /* PRIVATE_DSP_ARCH_AARCH64_ASIMD_FILTERS_STATIC_H_ */
