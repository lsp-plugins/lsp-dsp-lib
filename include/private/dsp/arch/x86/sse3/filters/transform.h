...
#ifndef PRIVATE_DSP_ARCH_X86_SSE3_FILTERS_TRANSFORM_H_
#define PRIVATE_DSP_ARCH_X86_SSE3_FILTERS_TRANSFORM_H_
...
namespace lsp
{
    ...
    {
        IF_ARCH_X86(
            static const float f_transform_const[] __lsp_aligned16 =
            {
                LSP_DSP_VEC4(1.0f)
            };
        );

        #define BILINEAR_TRANSFORM_X8(base, B0, B1, B2, A1, A2) \
            /* Load bottom part */ \
            ...
            __ASM_EMIT("movaps      " base " + 0x90(%[bc]), %[x10]") \
            __ASM_EMIT("movaps      " base " + 0xb0(%[bc]), %[x11]") \
            __ASM_EMIT("movaps      " base " + 0xd0(%[bc]), %[x12]") \
            __ASM_EMIT("movaps      " base " + 0xf0(%[bc]), %[x13]") \
            /* Transpose */ \
            ...
            __ASM_EMIT("movaps      %[x5],  0x00 + " A1 "(%[bf])") \
            __ASM_EMIT("movaps      %[x13], 0x10 + " A1 "(%[bf])") \
            __ASM_EMIT("movaps      %[x4],  0x00 + " A2 "(%[bf])") \
            __ASM_EMIT("movaps      %[x12], 0x10 + " A2 "(%[bf])") \
            ...
            __ASM_EMIT("movaps      " base " + 0x80(%[bc]), %[x10]") \
            __ASM_EMIT("movaps      " base " + 0xa0(%[bc]), %[x11]") \
            __ASM_EMIT("movaps      " base " + 0xc0(%[bc]), %[x12]") \
            __ASM_EMIT("movaps      " base " + 0xe0(%[bc]), %[x13]") \
            /* Transpose */ \
            __ASM_EMIT("movaps      %[x2], %[x6]")              /* x6 = b0[0] b1[0] b2[0]   ? */ \
            __ASM_EMIT("movaps      %[x10], %[x14]") \
            __ASM_EMIT("movaps      %[x4], %[x7]")              /* x7 = b0[2] b1[2] b2[2]   ? */ \
            ...
            /* Save top part */ \
            __ASM_EMIT("movaps      %[x4],  0x00 + " B0 "(%[bf])") \
            __ASM_EMIT("movaps      %[x12], 0x10 + " B0 "(%[bf])") \
            __ASM_EMIT("movaps      %[x3],  0x00 + " B1 "(%[bf])") \
            __ASM_EMIT("movaps      %[x11], 0x10 + " B1 "(%[bf])") \
            __ASM_EMIT("movaps      %[x2],  0x00 + " B2 "(%[bf])") \
            __ASM_EMIT("movaps      %[x10], 0x10 + " B2 "(%[bf])")
        ...
        void x64_bilinear_transform_x8(dsp::biquad_x8_t *bf, const dsp::f_cascade_t *bc, float kf, size_t count)
        {
            ...
            (
                ...
                __ASM_EMIT("1:")
                BILINEAR_TRANSFORM_X8("0x00", "0x00", "0x20", "0x40", "0x60", "0x80")
        ...
        void x64_bilinear_transform_x16(dsp::biquad_x16_t *bf, const dsp::f_cascade_t *bc, float kf, size_t count)
        {
            ...
            (
                ...
                __ASM_EMIT("1:")
                BILINEAR_TRANSFORM_X8("0x000", "0x00", "0x40", "0x80", "0xc0", "0x100")
                BILINEAR_TRANSFORM_X8("0x100", "0x20", "0x60", "0xa0", "0xe0", "0x120")
...
#endif /* PRIVATE_DSP_ARCH_X86_SSE3_FILTERS_TRANSFORM_H_ */
