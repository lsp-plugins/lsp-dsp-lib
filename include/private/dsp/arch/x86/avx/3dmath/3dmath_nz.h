    ...
    namespace avx
    {
        ...
        void apply_matrix3d_mm1(matrix3d_t *r, const matrix3d_t *m)
        {
            ...
            (
                ...
                // Save matrix (32-bit)
                __ASM_EMIT32("vmovaps     0x00(%[T]), %%xmm0")
                __ASM_EMIT32("vmovaps     0x10(%[T]), %%xmm1")
                __ASM_EMIT32("vmovaps     0x20(%[T]), %%xmm6")
                __ASM_EMIT32("vmovups     %%xmm0, 0x00(%[A])")
                __ASM_EMIT32("vmovups     %%xmm1, 0x10(%[A])")
        ...
        float calc_plane_p3(vector3d_t *v, const point3d_t *p0, const point3d_t *p1, const point3d_t *p2)
        {
            ...
            (
                ...
                __ASM_EMIT("vxorps          %[x2], %[x2], %[x2]")       // x2   = 0 0 0 0
                __ASM_EMIT("vmovups         (%[p0]), %[x3]")            // x3   = x0 y0 z0 w0
                __ASM_EMIT("vsubps          %[x1], %[x2], %[x2]")       // x2   = -nx -ny -nz 0
        ...
        float check_triplet3d_p3n_fma3(const point3d_t *p1, const point3d_t *p2, const point3d_t *p3, const vector3d_t *n)
        {
            ...
            (
                ...
                MIXED_PRODUCT3_FMA3("x0", "x1", "x2", "x3", "x4")
        ...
        float check_point3d_on_triangle_pvp(const point3d_t *pv, const point3d_t *p)
        {
            ...
            (
                ...
                /* x1 = r1 = m1 dot m2 */
                /* x2 = r2 = m2 dot m0 */
                __ASM_EMIT("vunpcklps       %[x1], %[x0], %[x0]")       // x0   = r0 r1 ? ?
                __ASM_EMIT("vmovlhps        %[x2], %[x0], %[x0]")       // x0   = r0 r1 r2 ?
                __ASM_EMIT("vxorps          %[x4], %[x4], %[x4]")       // x4   = 0
                __ASM_EMIT("vcmpps          $1, %[x4], %[x0], %[x3]")   // x3   = [ r0 < 0, r1 < 0, r2 < 0 ]
                __ASM_EMIT("vmovmskps       %[x3], %k[mask]")           // mask = r0 < 0 || r1 < 0 || r2 < 0
                __ASM_EMIT("testl           $0x07, %k[mask]")           // mask = r0 < 0 || r1 < 0 || r2 < 0
                __ASM_EMIT("jnz             100f")
                __ASM_EMIT("vmulss          %[x0], %[x1], %[x0]")       // x0   = r0 * r1
                __ASM_EMIT("vmulss          %[x0], %[x2], %[x0]")       // x0   = r0 * r1 * r2
                __ASM_EMIT("jmp             110f")
                __ASM_EMIT("100:")
                __ASM_EMIT("vmovss          %[NEG], %[x0]")             // x0   = negative value
                /* End */
                __ASM_EMIT("110:")
                : [x0] "=&x" (x0), [x1] "=&x" (x1), [x2] "=&x" (x2), [x3] "=&x" (x3),
                  [x4] "=&x" (x4), [x5] "=&x" (x5), [x6] "=&x" (x6), [x7] "=&x" (x7),
        ...
        float check_point3d_on_triangle_p3p(const point3d_t *p1, const point3d_t *p2, const point3d_t *p3, const point3d_t *p)
        {
            ...
            (
                ...
                __ASM_EMIT("jmp             110f")
                __ASM_EMIT("100:")
                __ASM_EMIT("vmovss          %[NEG], %[x0]")             // x0 = r1
                /* End */
                __ASM_EMIT("110:")
                : [x0] "=&x" (x0), [x1] "=&x" (x1), [x2] "=&x" (x2), [x3] "=&x" (x3),
                  [x4] "=&x" (x4), [x5] "=&x" (x5), [x6] "=&x" (x6), [x7] "=&x" (x7),
