...
#ifndef PRIVATE_DSP_ARCH_X86_SSE_FILTERS_FILL_H_
#define PRIVATE_DSP_ARCH_X86_SSE_FILTERS_FILL_H_
...
namespace lsp
{
    namespace sse
    {
        ...
        static const dsp::f_cascade_t fcascade_normal __lsp_aligned16 =
        {
            { 1.0f, 0.0f, 0.0f, 0.0f },
        };

        #define FCASCADE_FILL_X1(base, dst) \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x00(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x10(%[" dst "])")

        #define FCASCADE_FILL_X2(base, dst) \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x00(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x10(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x20(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x30(%[" dst "])")
        ...
        #define FCASCADE_FILL_X4(base, dst) \
            ...
            __ASM_EMIT("movups      %%xmm0, " base " + 0x20(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x30(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x40(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x50(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x60(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x70(%[" dst "])")
        ...
        #define FCASCADE_FILL_X8(base, dst) \
            ...
            __ASM_EMIT("movups      %%xmm0, " base " + 0x60(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x70(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x80(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0x90(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xa0(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xb0(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xc0(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xd0(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xe0(%[" dst "])") \
            __ASM_EMIT("movups      %%xmm0, " base " + 0xf0(%[" dst "])")

        void fcascade_fill_x2(dsp::f_cascade_t *h, dsp::f_cascade_t *t)
        {
            ARCH_X86_ASM(
                __ASM_EMIT("movaps      %[CC], %%xmm0")         // xmm0 = F
                // row 1
                FCASCADE_FILL_X1("0x20", "h")
                // row -1
                FCASCADE_FILL_X1("0x00", "t")
                :
                : [h] "r" (h), [t] "r" (t),
...
#endif /* PRIVATE_DSP_ARCH_X86_SSE_FILTERS_FILL_H_ */
