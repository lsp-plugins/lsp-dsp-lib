    ...
    namespace avx512
    {
        ...
        void corr_init(dsp::correlation_t *corr, const float *a, const float *b, size_t count)
        {
            ...
            (
                ...
                __ASM_EMIT("vmulps          %%zmm3, %%zmm5, %%zmm7")        /* zmm7 = a0*b0 */
                __ASM_EMIT("vmulps          %%zmm3, %%zmm3, %%zmm3")        /* zmm3 = a0*a0 */
                __ASM_EMIT("vmulps          %%zmm5, %%zmm5, %%zmm5")        /* zmm5 = b0*b0 */
                __ASM_EMIT("vaddps          %%zmm7, %%zmm0, %%zmm0")        /* zmm0 = xv + a0*b0 */
                __ASM_EMIT("vaddps          %%zmm3, %%zmm1, %%zmm1")        /* zmm1 = xa + a0*a0 */
                __ASM_EMIT("vaddps          %%zmm5, %%zmm2, %%zmm2")        /* zmm2 = xb + b0*b0 */
                __ASM_EMIT("vmulps          %%zmm4, %%zmm6, %%zmm7")        /* zmm7 = a0*b0 */
                __ASM_EMIT("vmulps          %%zmm4, %%zmm4, %%zmm4")        /* zmm4 = a0*a0 */
                __ASM_EMIT("vmulps          %%zmm6, %%zmm6, %%zmm6")        /* zmm6 = b0*b0 */
                __ASM_EMIT("vaddps          %%zmm7, %%zmm0, %%zmm0")        /* zmm0 = xv + a1*b1 */
                __ASM_EMIT("vaddps          %%zmm4, %%zmm1, %%zmm1")        /* zmm1 = xa + a1*a1 */
                __ASM_EMIT("vaddps          %%zmm6, %%zmm2, %%zmm2")        /* zmm2 = xb + b1*b1 */
